...
function Enemy:new(x, y, xVel, yVel)
	...
	self.movable = {
		...
		drag = { x = 0, y = reg.GRAVITY/2 }, -- 80 = gravity
		maxVelocity = { x = 80, y = 100 },
		speed = { x = 0, y = 0 } -- used to assign to acceleration
	}

	-- collider
	self.collider = HC:rectangle(self.pos.x - self.offset.x, self.pos.y - self.offset.y, reg.T_SIZE/2, reg.T_SIZE/2)
	...
	-- particles
	self.hitPs = love.graphics.newParticleSystem(assets.white, 100)
